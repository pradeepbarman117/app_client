...
const Pagination = ({ paginations }) => {
  const [currentPage, setCurrentPage] = useState(paginations?.currentPage);
  const totalPages = paginations?.totalPages; // You can adjust this based on your needs
  ...
  const renderPageNumbers = () => {
    ...
    if (currentPage > 3) {
      pages.push(
        <div
          key="start-ellipsis"
          className="hs-tooltip inline-block border border-gray-200"
        >
          <button
            type="button"
    ...
    // Show 1 or 2 pages before and after current page
