...
//       // Set the newlyAddedId state to trigger the flash animation
//       setNewlyAddedId({...updatedMaster});

//       // Reset newlyAddedId after the flash animation completes
//       setTimeout(() => {
//         setNewlyAddedId(null);
//       }, 1000); // 1000ms = 1 second (match this with your flash animation duration)

//       queryClient.setQueryData(["master/get"], (oldData) => {
//         if (!oldData) return { data: [updatedMaster] };

//         const existingMasterIndex = oldData.data.findIndex(
//           (master) => master.id === updatedMaster.id
//         );

//         if (existingMasterIndex !== -1) {
//           // If the master already exists, update only the specific fields
//           const updatedData = oldData.data.map((master, index) =>
//             index === existingMasterIndex
//               ? {
//                   ...master, // Preserve existing fields
//                   ...updatedMaster, // Apply updated fields
//                   admin: {
//                     ...master.admin, // Preserve existing admin fields
//                     ...(updatedMaster.admin || {}), // Apply updated admin fields if they exist
//                   },
//                 }
//               : master
//           );
...
//           };
//         } else {
//           // If it's a new master, add it to the list
//           return {
//             ...oldData,
//             data: [...oldData.data, updatedMaster],
//           };
//         }
//       });
//     };
...
//                         newlyAddedId?.id === items.id ? "flash" : ""
//                       }`}
//                     >
...
const Master = () => {
  ...
  // Memoizing the socket event handler to prevent re-registrations
  const handleMasterAdded = useCallback(
    (updatedMaster) => {
      setNewlyAddedId(updatedMaster);

      setTimeout(() => {
        setNewlyAddedId(null);
      }, 1000); // Match this with your flash animation duration
  ...
  // Memoize table rows to prevent unnecessary re-render
