...
import socketConfig from "../../services/socket/socket";
...
const Master = () => {
    ...
    useEffect(() => {
        ...
        const handleMasterUpdated = (updatedList) => {
            setMasters(updatedList);
        };

        socketConfig.io.on("masterAdded", handleMasterAdded);
        socketConfig.io.on("masterUpdated", handleMasterUpdated);

        return () => {
            socketConfig.io.off("masterAdded", handleMasterAdded);
            socketConfig.io.off("masterUpdated", handleMasterUpdated);
            if (timeoutIdRef.current) {
                clearTimeout(timeoutIdRef.current);
            }
        };
    ...
    return (
        <>
            ...
            <div className="flex justify-between gap-3 align-center px-4 pb-2 pt-4">
                ...
                <button type="button" className="text-gray-900 bg-white hover:bg-[#F0F2F5] border border-gray-200 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-white dark:border-gray-700 dark:text-gray-900"
                    onClick={toggleModal}
                >
                    <svg
                ...
                <div className="flex overflow-hidden rounded-xl border border-[#DBE0E5]">
                    <table className="flex-1">
                        ...
                        <tbody>
                            {
                                masters.map((items) => {
                                    return <tr key={items.id} className={`border-t hover:bg-[#12141705] cursor-pointer border-t-[#DBE0E5] ${newlyAddedId === items.id ? 'flash' : ''}`}>
